package com.zsj.server.registry.impl;

import com.zsj.server.registry.RegistryService;
import com.zsj.server.util.WatcherApi;
...
public class ZKRegistryService implements RegistryService {
    ...
    private static final String ROOT_PATH = "/IM-Server/" + "%s";
    ...
    public void registry()  throws Exception{
        ...
        String path = String.format(ROOT_PATH, ip + PORT);

        //如果节点不存在
        if(Objects.isNull(exists(path, true))){
            log.info("【执行初始化测试方法。。。。。。。。。。。。");
            createNode(path,new String("测试主服务器".getBytes(), CharsetUtil.UTF_8));
